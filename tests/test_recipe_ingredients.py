...
def test_update_recipe_ingredient(client: FlaskClient, auth_headers):
    ...
    assert response.status_code == 200 and response.json['quantity'] == 2 and response.json['unitOfMeasure'] == 'grams'

    response = get_recipe_ingredient(client, recipe_resp.json['_id'], tuna_resp.json['_id'], auth_headers)
